...
| Kursansichten und Übernahme standbewusst machen | I-14 | ❓ offen — **Stufe 2b/2c**. `CourseOverviewService`, `CourseGridService` und `CourseSequenceService` zeigen einen Umlauf als *eine* Folge; verzweigt er sich am Versionswechsel, stehen Fahrten beider Versionen untereinander, als führe das Fahrzeug sie am selben Tag. `CourseCarryoverService` überträgt beim Versionswechsel weiter genau einen Anschluss je Fahrt |
