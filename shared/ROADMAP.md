...
| Ketten, Kurse und Übernahme standbewusst machen | I-14 | ❓ offen — **Stufe 2** zum Punkt darüber. `chainFor`, `TripChainGraph`, `unifyChain`, Kurs-Überblick und `CourseCarryoverService` gehen noch davon aus, dass eine Kette eindeutig ist. Sie ist es nur noch innerhalb eines Versionsstands: Eine Fahrt mit zwei Anschlüssen liefert heute eine gemischte Kette, und die Kursnummer kann dadurch auf Fahrten wandern, die an diesen Tagen gar nicht zusammengehören. Auch die Zyklusprüfung ist noch tagblind — sie weist einen Ring ab, selbst wenn seine beiden Kanten an verschiedenen Tagen gelten |
