...
| Pflege-Reihenfolge Tram vor Bus | I-14 | ✅ entschieden 22.09.2026: Verkehrsmittel-Wahl **vor** der Haltestelle, sie filtert die Auswahlliste, hebt die Haltestellen mit offenen Anschlüssen hervor und bestimmt die Zählung im Editor (KURSE §2 K7). Die Gesamtzahl bleibt klein daneben stehen. Zählung in der Liste über die ganze Periode, nicht je Versionsstand — je Stand hieße, die Standfaltung für alle 60 Endstellen zu rechnen |
| Eine Fahrt braucht je Versionsstand einen anderen Anschluss | I-14 | ✅ entschieden 23.09.2026, **Stufe 1 umgesetzt**: Die Unique-Constraints auf `trip_links.from_trip_id`/`to_trip_id` sind gefallen, an ihre Stelle tritt eine Überschneidungsprüfung je **Betriebstag** (KURSE §3). Erledigt sind Schema, Zulässigkeit, Haltestellen-Editor und alle Zählungen |
| Ketten und Zyklusprüfung standbewusst machen | I-14 | ✅ **Stufe 2a** umgesetzt 23.09.2026: `TripChainGraph` führt Kanten als Listen mit ihren Tagen, `chainFor()` liefert die ganze verzweigte Menge (und mit Zeitraum den Zweig eines Stands), die Zyklusprüfung folgt nur Kanten, die die Tage der neuen berühren. Die Kursnummer erreicht damit beide Zweige — vorher fand die Kettenwanderung nur einen, und welchen, entschied die Zeilenreihenfolge |
| Kursansicht standbewusst machen | I-14 | ✅ **Stufe 2b** umgesetzt 23.09.2026: Kettenansicht und Tabelle tragen einen Versionsstand-Wähler, Fahrten und Anschlüsse werden darauf beschnitten. Die Faltung ist dafür aus dem Haltestellen-Editor in einen eigenen `VersionStandService` gewandert |
| Übernahme standbewusst machen | I-14 | ✅ **Stufe 2c** umgesetzt 23.09.2026: Ein Anschluss auf eine Fahrt außerhalb der gewechselten Version wird jetzt übertragen statt blockiert — die Gegenfahrt bekommt einen zweiten Anschluss auf die neue Version. Die Belegt-Prüfung beim Schreiben rechnet je Betriebstag, und was dabei liegen bleibt, erscheint unter `blocked` statt stillschweigend zu verschwinden |
| `CourseSequenceService` gegen verzweigte Ketten geprüft | I-14 | ✅ nachgemessen 23.09.2026: Er erfasst die verzweigte Kette vollständig und meldet beide Zweige unter `outside_range`. **Erst seit Stufe 2a** — vorher hielt der Kettengraph je Fahrt einen Nachfolger, und ein `clear` hätte die Nummer an der anderen Fahrt zurückgelassen. Durch einen Test festgenagelt |
| Paarung an Tauschpunkten (City Carré, Listemannstraße) | I-14 | ✅ entschieden 22.09.2026: **Schalter am Mengen-Lauf**, gepaart wird nur am selben Halt (KURSE §2 K7). Dort kommen mehrere Fahrten zeitgleich an und ab; die Zeit unterscheidet nichts, und die Datenbank-Id paarte die Kehrtwende. Bewusst ein Schalter statt einer Erkennung — geometrische Kriterien brauchen eine Winkelschwelle und liegen damit immer irgendwo daneben |
